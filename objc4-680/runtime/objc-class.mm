...
void _class_resolveMethod(Class cls, SEL sel, id inst)
{
    ...
    if (! cls->isMetaClass()) {
        ...
        /*
         可以发现调用类方法的时候，会再次兜底调用一次resolveInstanceMethod方法，
         也就是说如果调用一个不存在的类方法的时候，最终会调用一次NSObject的
         resolveInstanceMethod方法（这是因为resolveInstanceMethod
         调用的是当前对象的isa类中方法，而metaClass的isa一般都是NSObject）。
         所以在NSObject的resolveInstanceMethod中还会有一次补救机会。
         */
