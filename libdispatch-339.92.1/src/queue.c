...
/*
 dispatch_group_async的原理和dispatch_async比较类似，区别点在于group操作会带上
 DISPATCH_OBJ_GROUP_BIT标志位。添加group任务时会先执行dispatch_group_enter，
 然后在任务执行时会对带有该标记的执行dispatch_group_leave操作
