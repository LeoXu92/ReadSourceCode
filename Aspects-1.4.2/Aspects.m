...
// AspectsContainer是一个绑定在对象上的一个容器，
...
static BOOL aspect_isCompatibleBlockSignature(NSMethodSignature *blockSignature, id object, SEL selector, NSError **error) {
    ...
    if (blockSignature.numberOfArguments > methodSignature.numberOfArguments) {
        ...
        // 参数 0 是 slef 或者 block， 参数 1 是 SEL 或者 id<AspectInfo>
...
static void aspect_prepareClassAndHookSelector(NSObject *self, SEL selector, NSError **error) {
    ...
    if (!aspect_isMsgForwardIMP(targetMethodIMP)) {
        ...
        if (![klass instancesRespondToSelector:aliasSelector]) {
            // 添加 aspects__xxxx 方法，用于后面调用原有的方法。
            __unused BOOL addedAlias = class_addMethod(klass, aliasSelector, method_getImplementation(targetMethod), typeEncoding);
            // 断言，aspects__xxxx 这个方法 已经被实现了。
            NSCAssert(addedAlias, @"Original implementation for %@ is already copied to %@ on %@", NSStringFromSelector(selector), NSStringFromSelector(aliasSelector), klass);
        }

        // We use forwardInvocation to hook in.
        // 使用 _objc_msgForward 去调用 forwardInvocation
        // selector 替换成 _objc_msgForward
...
// 替换forwardInvocation方法的实现
static void aspect_swizzleForwardInvocation(Class klass) {
    NSCParameterAssert(klass);
    // If there is no method, replace will act like class_addMethod.
    // 如果没实现 forwardInvocation： 相当于执行 class_addMethod 方法
    IMP originalImplementation = class_replaceMethod(klass, @selector(forwardInvocation:), (IMP)__ASPECTS_ARE_BEING_CALLED__, "v@:@");
    if (originalImplementation) {
        // __aspects_forwardInvocation 指向的是原方法
        class_addMethod(klass, NSSelectorFromString(AspectsForwardInvocationSelectorName), originalImplementation, "v@:@");
    }
    AspectLog(@"Aspects: %@ is now aspect aware.", NSStringFromClass(klass));
}

/// 恢复forwardInvocation方法
...
static void __ASPECTS_ARE_BEING_CALLED__(__unsafe_unretained NSObject *self, SEL selector, NSInvocation *invocation) {
    ...
    // 如果没替换原始方法，需要调用原始forwardInvocation实现(通常抛出异常)
    if (!respondsToAlias) {
...
static BOOL aspect_isSelectorAllowedAndTrack(NSObject *self, SEL selector, AspectOptions options, NSError **error) {
    ...
    // Additional checks.
    // 检查dealloc方法是不是设置了除AspectPositionBefore以外的类型
    ...
    // 如果是hook的是类方法就要递归的去查父类有没有被追踪过
    if (class_isMetaClass(object_getClass(self))) {
        Class klass = [self class];
        // 已经hook了的类， key是class, value是AspectTracker
        ...
        do {
            // 先去缓存里找，这个类有没有hook过方法，从来没有hook过，创建一个AspectTracker实例，并加到缓存中
            tracker = swizzledClassesDict[currentClass];
            if (!tracker) {
