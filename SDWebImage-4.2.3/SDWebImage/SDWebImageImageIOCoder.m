...
/*
 这个不太好理解，我们先举个例子，比方说RGBA，其中R（红色）G（绿色）B（蓝色）A（透明度）是4个组件，
 每个像素由这4个组件组成，那么我们就用8位来表示着每一个组件，所以这个RGBA就是8*4 = 32位。
 知道了kBitsPerComponent和每个像素有多少组件组成就能计算kBytesPerPixel了。
 计算公式是：(bitsPerComponent * number of components + 7)/8.
 */
// 表示每一个组占多少位
 ...
 * 最大支持压缩图像源的大小默认的单位是MB，这里设置了60MB。当我们要压缩一张图像的时候，
   首先就是要定义最大支持的源文件的大小，不能没有任何限制。
 */
...
- (nullable UIImage *)sd_decompressedImageWithImage:(nullable UIImage *)image {
    ...
    @autoreleasepool{
        ...
        // 将图片转换为上下文并且转换为不包含alpha通道的 bitmap 图片
...
- (nullable UIImage *)sd_decompressedAndScaledDownImageWithImage:(nullable UIImage *)image {
    ...
    @autoreleasepool {
        ...
        // 计算源图像的重叠像素大小根据压缩后目标图像重叠像素大小来计算
        ...
        for( int y = 0; y < iterations; ++y ) {
            @autoreleasepool {
                ...
                // 从 sourceImageRef 切下 sourceTile
                ...
                // 绘制到 destContext 指定 destTile 位置
...
- (NSData *)encodedDataWithImage:(UIImage *)image format:(SDImageFormat)format {
    ...
    /**CGImageDestinationRef将图片数据写入目的地，并且负责做图片编码，
